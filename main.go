...
import (
	...
	"restaurant-system/internal/services/kitchen"
	"restaurant-system/internal/services/notification"
	"restaurant-system/internal/services/order"
	"restaurant-system/internal/services/tracking"
)
...
// runKitchenWorker runs the kitchen worker service
func runKitchenWorker(ctx context.Context, cfg *config.Config, log *logger.Logger, workerName, orderTypes string, heartbeatInterval, prefetch int) error {
	...
	// Parse order types
	parsedOrderTypes := models.ParseOrderTypes(orderTypes)

	// Create consumer for kitchen queue
	consumer := messaging.NewConsumer(conn, log, "kitchen_queue", workerName, prefetch)
	publisher := messaging.NewPublisher(conn, log)

	// Create and start kitchen worker
	worker := kitchen.NewWorker(
		workerName,
		parsedOrderTypes,
...
func runTrackingService(ctx context.Context, cfg *config.Config, log *logger.Logger, port int) error {
	...
	// Initialize service and handler
	service := tracking.NewService(db, log)
	handler := tracking.NewHandler(service, log)
	...
	// Start HTTP server in goroutine
	go func() {
		log.Info("service_started", fmt.Sprintf("Tracking Service started on port %d", port), requestID, map[string]interface{}{
			"port": port,
		})
		if err := server.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.Error("server_failed", "HTTP server failed", requestID, err, nil)
...
func runNotificationSubscriber(ctx context.Context, cfg *config.Config, log *logger.Logger) error {
	...
	// Create consumer for notifications queue
	consumer := messaging.NewConsumer(conn, log, "notifications_queue", "notification-subscriber", 1)

	// Create and start notification subscriber
	subscriber := notification.NewSubscriber(consumer, log)
